...
public final class SovereignAegisInput {
    ...
    public static boolean beginWheelCast(ResourceLocation parentId) {
        if (!isSupportedParent(parentId)) {
            return false;
        }
        activeSlot = WHEEL_SLOT;
        wheelParentId = parentId;
    ...
    public static void tickWheelCast(boolean confirmDown) {
        ...
        if (!confirmDown && wheelConfirmWasDown) {
            MagicalNetwork.sendWheelSubSkillCastRequest(wheelParentId, selectedMode);
            resetWheel();
            return;
        }
        if (confirmDown) {
            selectedMode = Math.floorMod(selectedMode, modesFor(wheelParentId).length);
            fade = Math.min(1.0F, fade + 0.18F);
        }
    ...
    private static ResourceLocation activeParentId() {
        return activeSlot == WHEEL_SLOT ? wheelParentId : ClientMagicState.get().equippedSkill(activeSlot);
    }
